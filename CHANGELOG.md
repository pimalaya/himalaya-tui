...
- 60-second idle ping issuing a NOOP to every registered network backend (IMAP, SMTP) when the user is inactive, so long reading sessions do not lose their connections to server-side inactivity timeouts.
